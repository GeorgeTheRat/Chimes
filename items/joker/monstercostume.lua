SMODS.Joker{ --Monster Costume
    ...
    discovered = true,
    atlas = "joker",
    pools = { ["chm_ostumes"] = true, ["cchm_stumes1"] = true },

    loc_vars = function(self, info_queue, card)
        
        local new_numerator, new_denominator = SMODS.get_probability_vars(card, 1, card.ability.extra.odds, "j_chm_onstercostume") 
    ...
    calculate = function(self, card, context)
        if context.individual and context.cardarea == G.play  then
            if true then
                if SMODS.pseudorandom_probability(card, "group_0_94b68a9b", 1, card.ability.extra.odds, "j_chm_onstercostume", false) then
    ...
    if #G.jokers.cards + G.GAME.joker_buffer < G.jokers.config.card_limit then
        ...
        G.GAME.joker_buffer = G.GAME.joker_buffer + 1
            G.E_MANAGER:add_event(Event({
                func = function()
                    local joker_card = SMODS.add_card({ set = "chm_ostumes2" })
                    if joker_card then
